 ...
 *	$config = array(
 *				'key' => 'Developers API Key', 						// API Key is required
 *				'token' => 'User Token',							// Optional. If supplied it will use previously created session
 *				)
 **/
/**
...
{//
	...
	// constants
	const session_identifier = 'ClockIt-SP';
	...
	public function __construct( $config = array() ) {
		try	{
			...
			// If we are passing in a token, then we don't need to re-create the session, as we want to use the one already instantiated
			// This allows us to use the ShiftPlanning API on multiple pages with the same session
			// Set the Developers Key
			$this->setAppKey($config['key']);

			if(!function_exists('curl_init' )) {// curl is not available
				throw new Exception( $this->internal_errors( 6 ) );
			}
			if(!function_exists('json_decode' )) {// json_decode is not available
	...
	{// store the user data to this session
		Yii::app()->user->setState('appToken',$this->response['token'][0]);
		Yii::app()->user->setState('appData',$this->response['data'][0]);
	}
	...
	{// destroy the currently active session
		...
		{// logout successful, remove local session data
			Yii::app()->user->setState('appToken', NULL);
			Yii::app()->user->setState('appData', NULL);
		}
		return $logout;
	}
	...
	{// check whether a valid session has been established
		if( Yii::app()->user->getState('appToken') != NULL )
		{// user is already authenticated
			return Yii::app()->user->getState('appData');
		}
		else
	...
	{// return the token that's currently being used
		...
		{//
			...
			{// user authenticated, return the token
				return Yii::app()->user->getState('appToken');
			}
			else
	...
	{// create the api call
		...
		{// session already established, use token
			...
			$this->request['token'] = Yii::app()->user->getState('appToken');
		}
		else
		...
		{//
			...
			{// response from API was a successful
				...
				{// debug mode is on
					...
					$db = fopen('/home/ethreal/www/dev.ethreal.net/log.txt', 'a');
					$tmp_vals = array( );
	...
	public function getShifts( $details = array ( ))
	{// get shifts
		return $this->setRequest(
			array_merge(
				array(
